...
const SHEETS = {
  ...
  Clients: ['Client ID','Client Name','Business Name','Location','Project Price','Advance Payment','Remaining Payment','Email','Phone Number','WordPress Username','WordPress Password','Payment Status','Created Date','Updated Date'],
  Payments: ['Payment ID','Client ID','Client Name','Business Name','Payment Amount','Remaining Payment','Payment Status','Payment Date'],
...
function ensureSheets_() {
  ...
  if (admin.getLastRow() < 2) { admin.appendRow([ADMIN_EMAIL, ADMIN_PASSWORD, 'Admin']); }
}
...
function deleteRow_(name, idKey, idValue) {
  ...
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(name);
  var vals = sh.getDataRange().getValues();
  var headers = vals[0]; var idx = headers.indexOf(idKey);
  for (var r = 1; r < vals.length; r++) {
    if (String(vals[r][idx]) === String(idValue)) { sh.deleteRow(r+1); return true; }
  }
  return false;
}
...
function uid_(prefix) { return prefix + '-' + Date.now() + '-' + Math.floor(Math.random()*1000); }
...
function doGet(e) {
  try {
    ...
    if (action === 'login') {
      var email = e.parameter.email, password = e.parameter.password;
...
function doPost(e) {
  try {
    ...
    if (action === 'addClient') {
      var c = body.data;
      var price = Number(c.projectPrice)||0, adv = Number(c.advancePayment)||0;
      var rem = price - adv;
      var id = uid_('CL');
      var now = nowIso_();
      appendRow_('Clients', {
        'Client ID': id, 'Client Name': c.clientName, 'Business Name': c.businessName,
        'Location': c.location, 'Project Price': price, 'Advance Payment': adv,
        'Remaining Payment': rem, 'Email': c.email, 'Phone Number': c.phone,
        'WordPress Username': c.wpUsername || '', 'WordPress Password': c.wpPassword || '',
        'Payment Status': rem <= 0 ? 'Completed' : 'Pending',
        'Created Date': now, 'Updated Date': now,
      });
      if (adv > 0) {
        appendRow_('Payments', {
          'Payment ID': uid_('PAY'), 'Client ID': id, 'Client Name': c.clientName,
          'Business Name': c.businessName, 'Payment Amount': adv,
          'Remaining Payment': rem, 'Payment Status': rem<=0?'Completed':'Pending',
          'Payment Date': now,
        });
      }
    ...
    if (action === 'updateClient') {
      var d = body.data;
      var price = Number(d.projectPrice)||0, adv = Number(d.advancePayment)||0;
      // recalc remaining from existing payments if paymentsRecalc
      var payments = readSheet_('Payments').filter(function(p){ return p['Client ID'] === d.clientId; });
      var paid = payments.reduce(function(s,p){ return s + (Number(p['Payment Amount'])||0); }, 0);
      ...
      updateRow_('Clients', 'Client ID', d.clientId, {
        'Client Name': d.clientName, 'Business Name': d.businessName, 'Location': d.location,
        'Project Price': price, 'Advance Payment': adv, 'Remaining Payment': rem,
        'Email': d.email, 'Phone Number': d.phone,
        'WordPress Username': d.wpUsername || '', 'WordPress Password': d.wpPassword || '',
        'Payment Status': rem<=0?'Completed':'Pending', 'Updated Date': nowIso_(),
      });
    ...
    if (action === 'receivePayment') {
      ...
      var client = clients.filter(function(x){ return x['Client ID'] === p.clientId; })[0];
      ...
      appendRow_('Payments', {
        'Payment ID': uid_('PAY'), 'Client ID': p.clientId,
        'Client Name': client['Client Name'], 'Business Name': client['Business Name'],
        'Payment Amount': amount, 'Remaining Payment': newRem,
        'Payment Status': status, 'Payment Date': nowIso_(),
      });
      var newAdv = (Number(client['Advance Payment'])||0) + amount;
      updateRow_('Clients', 'Client ID', p.clientId, {
        'Advance Payment': newAdv, 'Remaining Payment': newRem,
        'Payment Status': status, 'Updated Date': nowIso_(),
      });
    ...
    if (action === 'addExpense') {
      var ex = body.data;
      appendRow_('Expenses', {
        'Expense ID': uid_('EXP'), 'Expense Name': ex.name,
        'Expense Details': ex.details || '', 'Expense Amount': Number(ex.amount)||0,
    ...
    if (action === 'addDemo') {
      var dm = body.data;
      appendRow_('Demos', {
        'Demo ID': uid_('DEMO'), 'Business Name': dm.businessName,
        'Demo Link': dm.link, 'Added Date': nowIso_(),
      });
